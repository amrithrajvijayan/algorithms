...
public class ValidStartingCity {
	...
	public int validStartingCity(int[] distances, int[] fuel, int mpg) {
		...
		int startingCityCandidateIndex = 0;
		int milesRemainingAsStartingCityCandidate = 0;

		for (int index = 1; index < distances.length; index++) {
			int fuelFromPreviousCity = fuel[index - 1];
			int distanceFromPreCity = distances[index - 1];

			milesRemaining = milesRemaining + fuelFromPreviousCity * mpg - distanceFromPreCity;

			if (milesRemaining < milesRemainingAsStartingCityCandidate) {
				milesRemainingAsStartingCityCandidate = milesRemaining;
				startingCityCandidateIndex = index;
			}

		}

		return startingCityCandidateIndex;
	}
}
